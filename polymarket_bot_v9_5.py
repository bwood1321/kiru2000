...
class Dash:
    ...
    def render(s, c, conn, f, risk, mkt, strats, scores, orders, poly_pos,
        ...
        print(f"\n  {H1}╔{'═'*62}╗{R}")
        print(f"  {H1}║{R}  {H2}⬡ POLYMARKET BOT v9.5{R}          {DIM}{now}{R}  {DIM}⏱ {rt}{R}  {H1}║{R}")
        print(f"  {H1}║{R}  {DIM}₿ BTC 5m+15m{R}  {DIM}Ξ ETH 15m{R}  {DIM}◎ SOL 15m{R}  {DIM}✕ XRP 15m{R}   {H1}║{R}")
        ...
        # v9.5: Show feed status per asset
        feed_dots = ""
        if feeds:
        ...
        if cortex:
            ...
            # v9.5: Per-asset bias indicators
            asset_parts = []
            for a in ["btc", "eth", "sol", "xrp"]:
                ab = cortex._asset_bias.get(a, "NEUTRAL")
                ao = list(cortex._asset_outcomes.get(a, []))[-4:]
...
class Bot:
    ...
    def _init_history(s):
        ...
        with open(s.HISTORY_FILE, "a") as f:
            f.write(f"\n{'='*60}\n")
            f.write(f"  BOT v9.5 STARTED: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def _close_history(s):
        try:
            ...
            with open(s.HISTORY_FILE, "a") as f:
                f.write(f"{'─'*60}\n")
                f.write(f"  BOT v9.5 STOPPED: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def run(s):
        ...
        print(f"\n  {H1}{'='*55}{R}")
        print(f"  {'LIVE TRADING v9.5' if not s.c.dry_run else 'DRY RUN v9.5'}")
        print(f"  {H1}{'='*55}{R}")
        time.sleep(3); s._init_history(); s._sync_existing_positions(); s.dash.ev("Bot v9.5 started"); s._loop()
    ...
    def _summary(s):
        ...
        w, l, wr = s.risk.stats()
        print(f"\n{H1}{'═'*62}")
        print(f"  {LBL}SESSION SUMMARY — BOT v9.5 — MULTI-ASSET{R}")
        print(f"{H1}{'═'*62}{R}")
