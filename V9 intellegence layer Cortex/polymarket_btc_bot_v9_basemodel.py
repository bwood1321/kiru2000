        ...
        for p in s.positions:
            ...
            if resolved is not None:
                ...
                if btc_agrees is False:
                    log.warning(f"GAMMA vs BTC DISAGREE on {p.slug}! Gamma={resolved}, BTC={'up' if btc_up else 'down'}, side={p.side}. Using BTC.")
                    resolved = btc_says_won  # trust BTC over Gamma
    ...
    def _trade(s, m):
        ...
        def allowed(strat, side, price):
            ...
            if strat != "ARB":  # ARB is direction-agnostic
                ...
                # This would have prevented ALL 5 losing YES trades on Feb 17.
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        sz = sz * s.cortex.get_trust("LATENCY") * s.cortex.get_macro_mult(sig.get("dir", sig.get("side", "YES"))) * s.cortex.get_session_mult() * s.cortex.get_danger_mult() * s.cortex.get_side_mult(sig["dir"])  # v9: Cortex
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        conv_bonus = s.conviction.get_bonus(m.slug, sig["dir"])
                        sz = sz * conv_bonus * streak_mult * tod_mult * market_penalty
                        sz = sz * s.cortex.get_trust("MEANREV") * s.cortex.get_macro_mult(sig.get("dir", sig.get("side", "YES"))) * s.cortex.get_session_mult() * s.cortex.get_danger_mult() * s.cortex.get_side_mult(sig["dir"])  # v9: Cortex
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        conv_bonus = s.conviction.get_bonus(m.slug, sig["dir"])
                        sz = sz * conv_bonus * streak_mult * tod_mult * market_penalty
                        sz = sz * s.cortex.get_trust("FLASH") * s.cortex.get_macro_mult(sig.get("dir", sig.get("side", "YES"))) * s.cortex.get_session_mult() * s.cortex.get_danger_mult() * s.cortex.get_side_mult(sig["dir"])  # v9: Cortex
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
                        if sz >= 1.0:
                            bonus_tag = f" CONV:{conv_bonus:.1f}x" if conv_bonus > 1 else ""
            ...
            if sig and not s.sizer.is_paused("SQUEEZE"):
                ...
                if not ok:
                    ...
                    if not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        sz = sz * s.cortex.get_trust("SQUEEZE") * s.cortex.get_macro_mult(sig.get("dir", sig.get("side", "YES"))) * s.cortex.get_session_mult() * s.cortex.get_danger_mult() * s.cortex.get_side_mult(sig["dir"])  # v9: Cortex
