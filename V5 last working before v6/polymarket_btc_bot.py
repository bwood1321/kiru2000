...
class Config:
    ...
    latency_threshold: float = 0.005   # v5: higher threshold (was 0.004)
    latency_min_edge: float = 0.15     # v5: higher min edge (was 0.12)
    momentum_enabled: bool = True
    momentum_conf: float = 0.78        # v5: higher confidence (was 0.72)
    flash_enabled: bool = True
    flash_threshold: float = 0.18      # v5: lower threshold (was 0.20)
    max_daily_loss: float = 10.0       # v5: tighter stop loss
    max_positions: int = 3             # v5: fewer simultaneous (was 5)
    poll_sec: int = 2
...
class S_Latency:
    ...
    def check(s, m, f):
        ...
        up = chg > 0
        pred = min(0.90, 0.50 + abs(chg) * 35)  # v5: slightly less aggressive
        mp = m.yes_p if up else m.no_p
        edge = pred - mp
        if edge < s.c.latency_min_edge: return None
        if mp > 0.65: return None  # v5: tighter (was 0.70)
        other = m.no_p if up else m.yes_p
        if other < 0.30: return None  # v5: tighter (was 0.25)
...
class S_Momentum:
    ...
    def check(s, m, f):
        ...
        if conf < s.c.momentum_conf or abs(comp) < 0.18: return None  # v5: 0.18 (was 0.15)
        up = comp > 0
...
class S_Flash:
    """Buy extreme dips when BTC hasn't actually moved (panic selling)."""
    def __init__(s, c): s.c = c
    def check(s, m, f):
        ...
        if abs(f.chg(120)) > 0.012: return None  # v5: tighter (was 0.015)
        tl = (m.end - datetime.now(timezone.utc)).total_seconds()
        if tl < 240: return None  # v5: need more time (was 180)
        if yes_cheap:
...
class Executor:
    ...
    def connect(s, conn):
        ...
        # v5: Try proxy wallet first (type 1), then others
    ...
    def get_balance(s):
        """Get balance — check proxy wallet first, then signer, then CLOB API."""
        # For proxy wallets, check the funder (proxy) address
    ...
    def order(s, market, is_yes, price, size):
        ...
        try:
            ...
            if isinstance(resp, dict):
                ...
                log.info(f"ORDER: ${bet:.4f} {label} ({size:.2f}sh @ ${maker_price:.4f}) id={oid} st={status}")
                if oid != "?": return oid
            elif isinstance(resp, str) and len(resp) > 5:
                log.info(f"ORDER: ${bet:.4f} {label} resp={resp[:60]}")
                return resp
            return None
        except Exception as e:
    ...
    def redeem_positions(s, condition_ids):
        ...
        try:
            ...
            pk = s.c.private_key
            if not pk.startswith("0x"): pk = "0x" + pk
            addr = s.c.funder_address or Account.from_key(pk).address
            w3 = None
            for rpc in ["https://polygon-rpc.com", "https://rpc.ankr.com/polygon"]:
                try:
...
class Dash:
    ...
    def render(s, c, conn, f, risk, mkt, strats, scores, orders, poly_pos):
        ...
        print(f"  {H1}\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550")
        print(f"  \u2551  POLYMARKET BTC BOT v5 PRO   [{mode}]   {now}")
        print(f"  \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550{R}")
        g = f"{OK}\u2713{R}"; x = f"{ERR}\u2717{R}"
...
class Bot:
    def __init__(s):
        ...
        s.cd = {}; s._traded_cids = set()  # v5: track condition IDs for redeem
    ...
    def _trade(s, m):
        ...
        # Track condition ID for auto-redeem
