...
# ═══════════════════════════════════════════════════════════════
# ─── v8.1: THE MANAGER — Strategy Allocator ───
# ═══════════════════════════════════════════════════════════════
class StrategyManager:
    """Real-time portfolio manager that watches every strategy's performance
    and shifts capital toward winners. Unlike AdaptiveSizer (reacts after 5
    losses with a pause), The Manager updates trust scores on EVERY trade.
    
    Trust scores (0.25 to 2.0) multiply sizing:
    - Hot strategy (55%+ WR, positive ROI): 1.5-2.0x capital
    - Normal (45%+ WR): 1.0-1.3x
    - Cooling (35%+ WR): 0.5-0.8x  
    - Cold (<35% WR over 10+ trades): 0.25x (minimum — can earn back)
    
    Also detects momentum: 3 wins in a row = bonus, 3 losses = penalty.
    Persists across restarts by loading from trade_data.json."""

    def __init__(s):
        s._trades = {}   # {strat: deque of {"won": bool, "pnl": float, "size": float}}
        s._trust = {}    # {strat: float 0.25–2.0}
        s._strats = ["ARB", "LATENCY", "MEANREV", "FLASH", "SQUEEZE"]
        for st in s._strats:
            s._trades[st] = deque(maxlen=20)  # rolling window of last 20 trades
            s._trust[st] = 1.0  # start neutral

        # Try to load historical performance from trade_data.json
        s._load_history()

    def _load_history(s):
        """Bootstrap trust scores from existing trade history."""
        try:
    ...
    def record(s, strat, won, pnl, size):
        ...
        if strat not in s._trades:
            s._trades[strat] = deque(maxlen=20)
            s._trust[strat] = 1.0
        s._trades[strat].append({"won": won, "pnl": pnl, "size": size})
        s._recalc(strat)

    def _recalc(s, strat):
        """Recalculate trust score for a strategy."""
        ...
        # Use last 10 for primary scoring (or all if < 10)
        recent = trades[-10:]
        wins = sum(1 for t in recent if t["won"])
        total = len(recent)
        wr = wins / total
        ...
        # Base trust from win rate + ROI
        if wr >= 0.55 and roi > 0.10:
            # Hot — give more capital
            trust = 1.5 + min(roi * 2, 0.5)  # 1.5–2.0
        elif wr >= 0.45:
            # Normal — slight boost if profitable
            trust = 1.0 + min(max(roi, 0) * 1.5, 0.3)  # 1.0–1.3
        elif wr >= 0.35:
            # Cooling — reduce
            trust = 0.5 + wr  # 0.5–0.85
        else:
            # Cold — minimum capital
            trust = 0.25 + max(wr, 0) * 0.5  # 0.25–0.42

        # Momentum bonus: last 3 trades
        last3 = trades[-3:]
        ...
        s._trust[strat] = round(max(0.25, min(2.0, trust)), 2)

    def get_trust(s, strat):
        """Get the current trust multiplier for a strategy."""
        return s._trust.get(strat, 1.0)

    def display_str(s):
        """Dashboard display of all trust scores."""
        parts = []
        for st in s._strats:
            trust = s._trust.get(st, 1.0)
            n = len(s._trades.get(st, []))
            if n < 3:
                tag = f"{st[:3]}:--"
            elif trust >= 1.5:
                tag = f"{st[:3]}:🔥{trust:.1f}x"
            elif trust >= 1.0:
                tag = f"{st[:3]}:{trust:.1f}x"
            elif trust >= 0.5:
                tag = f"{st[:3]}:⚠{trust:.1f}x"
...
class S_Squeeze:
    ...
    def check(s, m, f, trend):
        ...
        yes_cheap = 0.10 <= m.yes_p <= 0.20
        no_cheap = 0.10 <= m.no_p <= 0.20
        if not yes_cheap and not no_cheap: return None
...
class Bot:
    ...
    def __init__(s):
        ...
        # v8.1: The Manager — real-time strategy allocator
        s.manager = StrategyManager()
    ...
    def run(s):
        ...
        print(f"    Strategy Manager: {OK}Active{R} (real-time trust scores, shifts capital to winners)")
        print(f"\n  {H1}{'='*55}{R}")
        ...
        while True:
            try:
                ...
                for p in resolved:
                    ...
                    # v8.1: Feed the Manager
                    s.manager.record(p.strat, p.pnl > 0, p.pnl, p.cost)
    ...
    def _trade(s, m):
        ...
        if sig and not s.sizer.is_paused("ARB"):
            ...
            if ok:
                ...
                sz = sz * s.manager.get_trust("ARB")  # v8.1: Manager trust
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        sz = sz * s.manager.get_trust("LATENCY")  # v8.1: Manager trust
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
        ...
        # But CAN fire independently from Flash — they look at different signals
        latency_open = any(p.strat == "LATENCY" and p.status == "OPEN" for p in open_here)
        # v8.1: Respect Latency's 60-second isolation period
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        conv_bonus = s.conviction.get_bonus(m.slug, sig["dir"])
                        sz = sz * conv_bonus * streak_mult * tod_mult * market_penalty
                        sz = sz * s.manager.get_trust("MEANREV")  # v8.1: Manager trust
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
            ...
            if s.momentum_guard.should_block(sig["yes"]):
                ...
                if not ok:
                    ...
                    if should and not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        conv_bonus = s.conviction.get_bonus(m.slug, sig["dir"])
                        sz = sz * conv_bonus * streak_mult * tod_mult * market_penalty
                        sz = sz * s.manager.get_trust("FLASH")  # v8.1: Manager trust
                        sz = min(sz, av, max_market_risk - market_risk, hard_max)
                        if sz >= 1.0:
                            bonus_tag = f" CONV:{conv_bonus:.1f}x" if conv_bonus > 1 else ""
            ...
            if sig and not s.sizer.is_paused("SQUEEZE"):
                ...
                if not ok:
                    ...
                    if not s.sizer.is_side_cold(sig["dir"]):
                        ...
                        sz = sz * s.manager.get_trust("SQUEEZE")  # v8.1: Manager trust
