"""
POLYMARKET BOT v11.1 — DATA-OPTIMIZED ENGINE
BTC (5m+15m) — 6 strategies from 30-day backtest validation

v11.1 STRATEGY ROSTER (all backtest-proven on 5000+ markets):
  1. ARB         — YES+NO arbitrage when sum < $0.95 (+$22K/30d)
  2. LATENCY     — BTC moved, market hasn't repriced. RETUNED: 0.08%/0.05 edge (+$1.7K)
  3. ACCEL ★     — BTC acceleration (rate-of-change increasing) (+$7-10K, star performer)
  4. DIVERGENCE  — BTC moved 0.10%+ but market stuck at 50/50 (+$3.2K, 63%WR!)
  5. MOMENTUM    — Sustained BTC trend across all timeframes (+$479, lowest MDD)
  6. OPENDRIVE   — Strong early market BTC moves 45-120s (+$162, bonus)

KILLED (proven losers across 5000+ market backtests):
  MeanRev (-$1K), Flash (-$380), SNIPE (0 trades), PAIR (-$5.8K),
  SPIKE (0 trades), FADE (-$5K), ENDGAME (-$368)

Architecture: Cortex intelligence + maker-first orders + anti-stall protections.
...
class Config:
    ...
    accel_pct: float = 0.05      # 5% — star performer from backtest
    divergence_pct: float = 0.05  # 5% — 63% WR strategy
    opendrive_pct: float = 0.03   # 3% — low volume bonus strategy
    ...
    def get_base_size(s, strat, balance):
        ...
        pct_map = {"ARB": s.arb_pct, "LATENCY": s.latency_pct,
                   "MOMENTUM": s.momentum_pct, "ACCEL": s.accel_pct,
                   "DIVERGENCE": s.divergence_pct, "OPENDRIVE": s.opendrive_pct}
        fixed_map = {"ARB": s.arb_size, "LATENCY": s.latency_size,
                     "MOMENTUM": s.momentum_size, "ACCEL": 2.0, "DIVERGENCE": 2.0}
...
class TrendEngine:
    ...
    def should_trade(s, strat, side_is_yes):
        ...
        # ACCEL: works best in trending/breakout, quiet in flat
        if strat == "ACCEL":
        ...
        # MOMENTUM/DIVERGENCE/OPENDRIVE: trend-based, need directional signal, but don't fully block in FLAT
        if strat in ("MOMENTUM", "DIVERGENCE", "OPENDRIVE"):
...
class AdaptiveSizer:
    ...
    def __init__(s, c):
        ...
        s.multipliers = {"ARB": 1.0, "LATENCY": 1.0, "ACCEL": 1.0, "DIVERGENCE": 1.0, "MOMENTUM": 1.0, "OPENDRIVE": 1.0}
        s.paused_until = {"ARB": 0, "LATENCY": 0, "ACCEL": 0, "DIVERGENCE": 0, "MOMENTUM": 0, "OPENDRIVE": 0}
        s.streaks = {"ARB": 0, "LATENCY": 0, "ACCEL": 0, "DIVERGENCE": 0, "MOMENTUM": 0, "OPENDRIVE": 0}  # negative = losses
    ...
    def _recalculate(s):
        ...
        for strat in ["ARB", "LATENCY", "ACCEL", "DIVERGENCE", "MOMENTUM", "OPENDRIVE"]:
...
class Cortex:
    ...
    STRATS = ["ARB", "LATENCY", "ACCEL", "DIVERGENCE", "MOMENTUM", "OPENDRIVE"]
    ...
    REGIME_AFFINITY = {
        ...
        "ACCEL":      {"TRENDING_UP": 1.3, "TRENDING_DOWN": 1.3, "BREAKOUT": 1.5, "FLAT": 0.5, "CHOPPY": 0.7},
        "DIVERGENCE": {"TRENDING_UP": 1.2, "TRENDING_DOWN": 1.2, "BREAKOUT": 1.3, "FLAT": 0.6, "CHOPPY": 0.8},
        "MOMENTUM":   {"TRENDING_UP": 1.5, "TRENDING_DOWN": 1.5, "BREAKOUT": 1.3, "FLAT": 0.3, "CHOPPY": 0.5},
        "OPENDRIVE":  {"TRENDING_UP": 1.2, "TRENDING_DOWN": 1.2, "BREAKOUT": 1.4, "FLAT": 0.5, "CHOPPY": 0.7},
    ...
    def __init__(s):
        ...
        # Keys like "ACCEL:btc-15m", "LATENCY:btc-5m", "DIVERGENCE:btc-15m"
        s._slot_trades = {}   # {"ACCEL:btc-15m": deque(maxlen=15)}
        s._slot_trust = {}    # {"ACCEL:btc-15m": 1.0}
    ...
    def _recalc_slot_trust(s, sk, strat):
        ...
        if strat in ("LATENCY", "ACCEL"): trust = max(1.0, trust)
        if n >= 6 and wins == 0: trust = 0.0
        s._slot_trust[sk] = round(max(0.0, min(2.5, trust)), 2)
        if strat not in ("LATENCY", "ACCEL", "DIVERGENCE") and s._slot_trust[sk] < 0.40 and s._slot_trust[sk] > 0.0:
    ...
    def get_max_entry(s, strat):
        ...
        OPENDRIVE has its own entry logic, trust can't override."""
        if strat == "OPENDRIVE": return 0.50  # OpenDrive in early market
        trust = s._trust.get(strat, 1.0)
        defaults = {"ARB": 0.50, "LATENCY": 0.50, "ACCEL": 0.50, "DIVERGENCE": 0.55, "MOMENTUM": 0.50, "OPENDRIVE": 0.45}
...
class S_Latency:
    ...
    v11.1: AGGRESSIVE tuning from COMBO B backtest (+$1,698, 57%WR):
    - Threshold: 0.08% (was 0.15%) — catches more real signals
    - Edge minimum: 0.05 (was 0.08) — lower bar when direction is clear
    - Price range: $0.10-$0.50 (was $0.15-$0.45) — wider to catch more
    - Breakout mode: 0.06% threshold (even more sensitive)
    
    KEY INSIGHT: When MeanRev and Flash aren't competing for positions,
    Latency goes from 34%WR to 57%WR. The other strategies were
    creating noise that made Latency entries worse."""
    ...
    def check(s, m, f, trend):
        ...
        min_tl = 90 if m.timeframe == 5 else 180
        if tl < min_tl: return None

        # PRIMARY signal = move from market open (this is what settles)
        chg_open = 0
        if m.open_btc > 0:
        ...
        # SECONDARY = recent momentum (confirms direction)
        binance_chg_30 = f.chg(30)
        binance_chg_60 = f.chg(60)
        
        chg = chg_open
        if abs(chg_open) < 0.0003:
            return None
        
        # v11.1: AGGRESSIVE thresholds from COMBO B backtest
        # 0.08% normal, 0.06% in breakout (was 0.15%/0.10%)
        min_chg = 0.0006 if (trend and trend.regime == "BREAKOUT") else 0.0008
        if abs(chg) < min_chg: return None
        
        # Secondary momentum must AGREE
        if binance_chg_30 != 0:
        ...
        # v11.1: Wider range $0.10-$0.50 (was $0.15-$0.45)
        if target_price > 0.50 or target_price < 0.10: return None

        other_price = m.no_p if up else m.yes_p
        if other_price < 0.08: return None

        # v11.1: Confidence scaled to BTC move
        ...
        # v11.1: Lower edge bar — 0.05 (was 0.08)
...
class S_Accel:
    """ACCELERATION — BTC not just moving but ACCELERATING.
    
    BACKTEST PROVEN (30 days, 5316 markets):
      0.05% threshold: 581t, 45%WR, +$7,352  ← STAR PERFORMER
      In BTC-based combo: 570t, 45%WR, +$10,828
    
    The insight: raw BTC movement is noise. But when the RATE of change
    is increasing (30s change > 60s change), it signals genuine momentum
    that Polymarket markets haven't priced in yet.
    
    Parameters from backtest winner:
    - Acceleration threshold: 0.05% (30s change must exceed this)
    - 30s change must be > 60s average rate (true acceleration)
    - Both Chainlink and Binance must agree on direction
    - Entry price: $0.10-$0.50 (cheap side has better payoff ratio)
    """
    def __init__(s, c): s.c = c
    def check(s, m, f, trend):
        if f.n < 15: return None
        tl = (m.end - datetime.now(timezone.utc)).total_seconds() if m.end else 999
        min_tl = 60 if m.timeframe == 5 else 120
        if tl < min_tl: return None
        
        # Calculate acceleration: 30s change vs 60s change rate
        chg_30 = f.chg(30)
        chg_60 = f.chg(60)
        
        if abs(chg_30) < 0.0005:  # 0.05% minimum 30s move
            return None
        
        # True acceleration: 30s rate > 60s rate (momentum increasing)
        rate_30 = chg_30 / 30.0   # per-second rate
        rate_60 = chg_60 / 60.0   # per-second rate
        
        # Same direction AND accelerating
        if chg_30 > 0 and rate_30 > rate_60 and rate_30 > 0:
            accel = True
            up = True
        elif chg_30 < 0 and rate_30 < rate_60 and rate_30 < 0:
            accel = True
            up = False
        else:
            return None
        ...
        # Chainlink agreement
        cl_chg = f.cl_chg(30) if hasattr(f, 'cl_chg') else 0
        if cl_chg != 0:
            if (cl_chg > 0) != up:
                return None
        ...
        # Confirm with open price direction
        if m.open_btc > 0:
            settle = f.settlement_price if hasattr(f, 'settlement_price') else f.price
            if settle > 0:
                chg_open = (settle - m.open_btc) / m.open_btc
                # Open direction must agree with acceleration
                if (chg_open > 0) != up and abs(chg_open) > 0.0003:
                    return None
        
        return {"s": "ACCEL", "dir": "YES" if up else "NO", "yes": up,
                "p": target_price, "accel": abs(rate_30 - rate_60), "sz": 0}


class S_Divergence:
    """DIVERGENCE — BTC moved but Polymarket price still near 50/50.
    
    BACKTEST PROVEN (30 days, 5316 markets):
      0.10% BTC threshold: 323t, 63%WR, +$3,221  ← HIGHEST WR
    
    Different from Latency: Divergence specifically requires the market
    to be STILL near 50/50 (both sides $0.40-$0.60) while BTC has already
    moved 0.10%+. This is a pure "market hasn't caught up" play.
    
    Parameters from backtest winner:
    - BTC must have moved 0.10%+ from open
    - Market prices still $0.40-$0.60 range (hasn't adjusted)
    - Buy the side that SHOULD win based on BTC direction
    """
    def __init__(s, c): s.c = c
    def check(s, m, f, trend):
        ...
        # BTC must have moved 0.10%+ from open
        if m.open_btc <= 0: return None
        settle = f.settlement_price if hasattr(f, 'settlement_price') else f.price
        if settle <= 0: return None
        
        chg_open = (settle - m.open_btc) / m.open_btc
        if abs(chg_open) < 0.0010:  # 0.10% minimum
            return None
        
        up = chg_open > 0
        
        # KEY FILTER: Market hasn't adjusted yet
        # Both sides still in $0.40-$0.60 range = market is "stuck"
        if not (0.35 <= m.yes_p <= 0.65 and 0.35 <= m.no_p <= 0.65):
            return None  # Market already moved, no divergence
        
        # The divergence: BTC clearly up/down but market at ~50/50
        target_price = m.yes_p if up else m.no_p
        
        # Must still be reasonably priced (not already catching up)
        if target_price > 0.55 or target_price < 0.35: return None
        
        # Chainlink agreement
        cl_chg = f.cl_chg(60) if hasattr(f, 'cl_chg') else 0
        if cl_chg != 0:
            if (cl_chg > 0) != up:
                return None
        
        # Recent momentum should confirm (not reversing)
        chg_30 = f.chg(30)
        if (chg_30 > 0) != up and abs(chg_30) > 0.0003:
            return None  # BTC reversing
        
        divergence_size = abs(chg_open)  # How much BTC moved
        
        return {"s": "DIVERGENCE", "dir": "YES" if up else "NO", "yes": up,
                "p": target_price, "diverge": divergence_size, "sz": 0}


class S_Momentum:
    """MOMENTUM — BTC trending strongly, ride the trend.
    
    BACKTEST PROVEN (30 days, 5316 markets):
      0.10% threshold: 48t, 56%WR, +$479 (LOW MDD 10%!)
    
    Different from Accel: Momentum requires SUSTAINED movement across
    multiple timeframes (30s, 60s, open all agree). Accel just needs
    increasing rate. Momentum = "the trend is your friend" play.
    
    Parameters from backtest winner:
    - All timeframes agree: 30s, 60s, and open direction same
    - Minimum 0.10% move on each timeframe
    - Entry price: $0.10-$0.50
    """
    def __init__(s, c): s.c = c
    def check(s, m, f, trend):
        if f.n < 15: return None
        ...
        # ALL three must agree AND be significant (0.10%+)
        min_move = 0.0010  # 0.10%
        
        if chg_30 > min_move and chg_60 > min_move and chg_open > min_move:
            up = True
        elif chg_30 < -min_move and chg_60 < -min_move and chg_open < -min_move:
            up = False
        else:
            return None  # No clear sustained trend
        ...
        # Trend engine should agree if available
        if trend:
            if up and trend.trend_dir < -0.3: return None  # Trend engine says down
            if not up and trend.trend_dir > 0.3: return None  # Trend engine says up
        
        strength = min(abs(chg_30), abs(chg_60), abs(chg_open))
        
        return {"s": "MOMENTUM", "dir": "YES" if up else "NO", "yes": up,
                "p": target_price, "strength": strength, "sz": 0}


class S_OpenDrive:
    """OPENDRIVE — BTC moves hard in first 45-120s of market.
    
    BACKTEST PROVEN (30 days, 5316 markets):
      45-120s window, 0.15% move: 7t, 57%WR, +$162
    
    Early momentum in a market tends to persist. If BTC moves 0.15%+
    within the first 45-120 seconds, the market likely hasn't priced
    it in yet (everyone's still watching).
    
    Parameters from backtest winner:
    - Market age: 45-120 seconds (very early)
    - BTC move from open: 0.15%+ (strong opening)
    - Entry price: $0.10-$0.50
    """
    def __init__(s, c): s.c = c
    def check(s, m, f, trend):
        ...
        # Only fire in early market: 45-120 seconds after open
        if market_age < 45 or market_age > 120: return None
        
        # BTC must have moved 0.15%+ from market open
        if m.open_btc <= 0: return None
        settle = f.settlement_price if hasattr(f, 'settlement_price') else f.price
        if settle <= 0: return None
        
        chg_open = (settle - m.open_btc) / m.open_btc
        if abs(chg_open) < 0.0015:  # 0.15% minimum
            return None
        ...
        # Quick 30s check — momentum should still be going same way
        chg_30 = f.chg(30)
        if (chg_30 > 0) != up and abs(chg_30) > 0.0005:
            return None  # Early reversal
        
        return {"s": "OPENDRIVE", "dir": "YES" if up else "NO", "yes": up,
                "p": target_price, "drive": abs(chg_open), "sz": 0}
...
class Dash:
    ...
    def render(s, c, conn, f, risk, mkt, strats, scores, orders, poly_pos,
               ...
               cortex=None, poly_ws=None, slot_markets=None, feeds=None, active_slot="", bot_hb=None):
        ...
        print(f"\n  {H1}╔{'═'*62}╗{R}")
        print(f"  {H1}║{R}  {H2}⬡ POLYMARKET BOT v11{R}             {DIM}{now}{R}  {DIM}⏱ {rt}{R}  {H1}║{R}")
        print(f"  {H1}║{R}  {DIM}₿ BTC 5m+15m{R}  {DIM}v11.1 OPTIMIZED (6 strategies, data-driven){R}         {H1}║{R}")
        ...
        if trend:
            ...
            # v11 heartbeat: compact status next to AI clock
            _hb = bot_hb or {}
            _sa = int(time.time() - _hb.get("scan", 0)) if _hb.get("scan", 0) > 0 else -1
            _ta = int(time.time() - _hb.get("trade", 0)) if _hb.get("trade", 0) > 0 else -1
            _tn = _hb.get("trades", 0)
            _sn = _hb.get("scans", 0)
            ...
            print(f"  {LBL}AI{R}   {rc}{arrow} {regime}{R}  {DIM}⏱{R} {rt}  {hb} {DIM}{_sn}scn{R} {DIM}{_tn}trd{R}({tt})" if rt else f"  {LBL}AI{R}   {rc}{arrow} {regime}{R}  {hb} {DIM}{_sn}scn{R} {DIM}{_tn}trd{R}({tt})")
        ...
        print(f"  {H1}├{'─'*62}┤{R}")
        icons = {"ARB": "♦", "LATENCY": "⚡", "ACCEL": "⏩", "DIVERGENCE": "↔", "MOMENTUM": "→", "OPENDRIVE": "▶"}
        ...
        if evts:
            ...
            for e in evts:
                if "WON" in e or "REDEEMED" in e or "ACTIVE" in e:
                    ...
                    print(f"    {ERR}{e}{R}")
                elif "LAT" in e or "ACCEL" in e or "DIV" in e:
                    print(f"    {OK}{e}{R}")
                else:
                    print(f"    {DIM}{e}{R}")
...
class Bot:
    ...
    def __init__(s):
        ...
        s.s1 = S_Arb(s.c); s.s2 = S_Latency(s.c); s.s3 = S_Accel(s.c); s.s4 = S_Divergence(s.c); s.s5 = S_Momentum(s.c); s.s6 = S_OpenDrive(s.c)
        s.mkt = None; s.strats = {"ARB": "...", "LATENCY": "...", "ACCEL": "...", "DIVERGENCE": "...", "MOMENTUM": "...", "OPENDRIVE": "..."}
        ...
        # v11 HEARTBEAT — tracks activity so dashboard shows bot is alive
        s._hb_last_scan = 0        # timestamp of last market scan
        s._hb_last_trade = 0       # timestamp of last trade placed
        s._hb_scans = 0            # total scans this session
        s._hb_signals = 0          # signals seen (before filters)
        s._hb_trades = 0           # trades placed this session
        s._hb_gate_skips = 0       # times BTC gate blocked a trade
        s._hb_price_skips = 0      # times price filter blocked
        s._hb_last_signal = ""     # last signal description
    ...
    def _beep(s):
        """v11: Beep + heartbeat update on trade."""
        s._hb_last_trade = time.time()
        s._hb_trades += 1
        try:
    ...
    def run(s):
        ...
        os.system("cls" if os.name == "nt" else "clear")
        print(f"\n  {H1}{'='*55}\n  |  POLYMARKET BOT v11.1 — DATA-OPTIMIZED\n  |  BTC 5m+15m | 6 Strategies | Backtest-Proven\n  {'='*55}{R}\n")
        ...
        while True:
            try:
                ...
                if ctr % 100 == 0:
                    ...
                    gc.collect()
                try:
                    s.dash.render(s.c, s.conn, s.feeds.get("btc", s.feed), s.risk, s.mkt, s.strats, {},
                        ...
                        bot_hb={"scan": s._hb_last_scan, "trade": s._hb_last_trade,
                                "scans": s._hb_scans, "trades": s._hb_trades})
                except Exception as e:
    ...
    def _trade(s, m):
        ...
        # RULE 1: BTC must move 0.10%+ from market open to trade
        # When BTC is flat, ALL strategies lose money. Period.
        # This is the #1 cause of losses — trading on noise.
        _btc_moved = False
        _chg_from_open = 0
        if m.open_btc > 0 and s.feed.price > 0:
            _chg_from_open = abs((s.feed.price - m.open_btc) / m.open_btc)
            _btc_moved = _chg_from_open >= 0.0008  # v11.1: lowered to 0.08% — COMBO B backtest best
        ...
        if time.time() - s._state_times.get(_st_key, 0) > 30:
            ...
            open_pos = len([p for p in s.risk.positions if p.status == "OPEN" and p.slug == m.slug])
            paused = [st for st in ["ARB","LATENCY","ACCEL","DIVERGENCE","MOMENTUM","OPENDRIVE"] if s.sizer.is_paused(st)]
            log.info(f"v11STATE {m.asset}-{m.timeframe}m Y={m.yes_p:.2f} N={m.no_p:.2f} "
                     f"btc_chg={_chg_from_open*100:+.3f}% moved={_btc_moved} "
                     f"open={open_pos} pen={market_penalty:.1f} av=${av:.0f} "
        ...
        if spread is not None and spread < 0.90 and spread > 0.08:
            ...
            s.strats["LATENCY"] = "wide spread"; s.strats["ACCEL"] = "wide spread"
            s.strats["DIVERGENCE"] = "wide spread"; s.strats["MOMENTUM"] = "wide spread"
            s.strats["OPENDRIVE"] = "wide spread"
            return
        ...
        hard_max = min(s.risk.show_bal * 0.08, 300)  # v11: was 10%/$400. Data: $200-400=+$2,209, $400+=-$1,337
        ...
        if time.time() - s._debug_times.get(_dbg2_key, 0) > 30:
            ...
            log.info(f"v10STATE {m.asset}-{m.timeframe}m "
                     ...
                     f"paused={[k for k in ['ARB','LATENCY','ACCEL','DIVERGENCE','MOMENTUM','OPENDRIVE'] if s.sizer.is_paused(k)]}")
        ...
        def allowed(strat, side, price):
            ...
            if strat != "ARB" and not _cross_slot_ok(side):
            ...
            # Counter-trend sizing reduction
            ...
            if open_count + resolved_count >= 2:
                return False, "market cap (2 trades)", 0
            # Market risk cap
        ...
        # v11: BTC must have moved 0.10%+ from open for directional strategies
        sig = s.s2.check(m, s.feed, s.trend)
        if sig and lat_ok and _btc_moved and time.time() - s.cd.get(f"lat:{slot_key}", 0) > 15 and not s.sizer.is_paused("LATENCY"):
            p = sig["p"]
            ...
            if not ok:
                ...
                sz = sz * market_penalty * _cl_mult
                sz = min(sz, av, max_market_risk - market_risk, hard_max)
                if sz >= 1.0 and 0.10 <= p <= 0.50:  # v11.1: widened from COMBO B backtest (57%WR, +$1,698)
        ...
        # ── S3: ACCEL (BTC acceleration, star performer +$7-10K) ──
        # v11.1 NEW: Backtest proven 581t, 45%WR, +$7,352. No BTC gate needed (internal check).
        sig = s.s3.check(m, s.feed, s.trend)
        if sig and time.time() - s.cd.get(f"accel:{slot_key}", 0) > 10 and not s.sizer.is_paused("ACCEL"):
            p = sig["p"]
            ok, reason, same_count = allowed("ACCEL", sig["dir"], p)
            if not ok:
                s.strats["ACCEL"] = reason
            else:
                sz = s.c.get_base_size("ACCEL", s.risk.show_bal)
                sz = sz * market_penalty * _cl_mult
                sz = min(sz, av, max_market_risk - market_risk, hard_max)
                if sz >= 1.0 and 0.10 <= p <= 0.50:
                    s.strats["ACCEL"] = f"ACTIVE {sig['dir']} accel={sig.get('accel',0)*10000:.1f}bps/s"
                    sh = max(sz / p, 5.0)
                    if sh * p > av: sh = av / p
                    s.dash.ev(f"[{slot_label}·ACCEL] {sig['dir']} ${sz:.2f} @${p:.2f}")
                    oid, actual_shares = s.ex.order(m, sig["yes"], p, sh, mode="maker")
                    if oid:
                        t = Trd(datetime.now(timezone.utc), "ACCEL", m.slug, sig["dir"], p, sz, oid=oid)
                        s.risk.open(t, market_end=m.end, actual_shares=actual_shares, entry_regime=s.trend.regime if s.trend else "UNKNOWN"); s.cd[f"accel:{slot_key}"] = time.time()
                        s._recent_entries.append((time.time(), t.side, slot_key))
                        if m.cid: s._traded_cids.add(m.cid); s._save_traded_cids()
                        s.conviction.record_signal(m.slug, "ACCEL", sig["dir"])
                        s._beep()
                else: s.strats["ACCEL"] = f"signal! {sig['dir']} p=${p:.2f}"
        else:
            s.strats["ACCEL"] = s.strats.get("ACCEL", "monitoring")

        # ── S4: DIVERGENCE (BTC moved, market stuck at 50/50, 63%WR!) ──
        # v11.1 NEW: Backtest proven 323t, 63%WR, +$3,221
        sig = s.s4.check(m, s.feed, s.trend)
        if sig and time.time() - s.cd.get(f"div:{slot_key}", 0) > 15 and not s.sizer.is_paused("DIVERGENCE"):
            p = sig["p"]
            ok, reason, same_count = allowed("DIVERGENCE", sig["dir"], p)
            if not ok:
                s.strats["DIVERGENCE"] = reason
            else:
                sz = s.c.get_base_size("DIVERGENCE", s.risk.show_bal)
                sz = sz * market_penalty * _cl_mult
                sz = min(sz, av, max_market_risk - market_risk, hard_max)
                if sz >= 1.0:
                    s.strats["DIVERGENCE"] = f"ACTIVE {sig['dir']} div={sig.get('diverge',0)*100:.2f}%"
                    sh = max(sz / p, 5.0)
                    if sh * p > av: sh = av / p
                    s.dash.ev(f"[{slot_label}·DIV] {sig['dir']} ${sz:.2f} @${p:.2f} div={sig.get('diverge',0)*100:.2f}%")
                    oid, actual_shares = s.ex.order(m, sig["yes"], p, sh, mode="maker")
                    if oid:
                        t = Trd(datetime.now(timezone.utc), "DIVERGENCE", m.slug, sig["dir"], p, sz, oid=oid)
                        s.risk.open(t, market_end=m.end, actual_shares=actual_shares, entry_regime=s.trend.regime if s.trend else "UNKNOWN"); s.cd[f"div:{slot_key}"] = time.time()
                        s._recent_entries.append((time.time(), t.side, slot_key))
                        if m.cid: s._traded_cids.add(m.cid); s._save_traded_cids()
                        s.conviction.record_signal(m.slug, "DIVERGENCE", sig["dir"])
                        s._beep()
                else: s.strats["DIVERGENCE"] = f"signal! {sig['dir']} sz too small"
        else:
            s.strats["DIVERGENCE"] = s.strats.get("DIVERGENCE", "monitoring")

        # ── S5: MOMENTUM (strong sustained BTC trend, 56%WR, low MDD) ──
        # v11.1 NEW: Backtest proven 48t, 56%WR, +$479, only 10% MDD
        sig = s.s5.check(m, s.feed, s.trend)
        if sig and time.time() - s.cd.get(f"mom:{slot_key}", 0) > 20 and not s.sizer.is_paused("MOMENTUM"):
            p = sig["p"]
            ...
            if not ok:
                ...
                sz = min(sz, av, max_market_risk - market_risk, hard_max)
                if sz >= 1.0 and 0.10 <= p <= 0.50:
                    s.strats["MOMENTUM"] = f"ACTIVE {sig['dir']} str={sig.get('strength',0)*100:.2f}%"
                    sh = max(sz / p, 5.0)
                    if sh * p > av: sh = av / p
                    s.dash.ev(f"[{slot_label}·MOM] {sig['dir']} ${sz:.2f} @${p:.2f}")
                    oid, actual_shares = s.ex.order(m, sig["yes"], p, sh, mode="maker")
                    if oid:
                        t = Trd(datetime.now(timezone.utc), "MOMENTUM", m.slug, sig["dir"], p, sz, oid=oid)
                        s.risk.open(t, market_end=m.end, actual_shares=actual_shares, entry_regime=s.trend.regime if s.trend else "UNKNOWN"); s.cd[f"mom:{slot_key}"] = time.time()
                        s._recent_entries.append((time.time(), t.side, slot_key))
                        if m.cid: s._traded_cids.add(m.cid); s._save_traded_cids()
            ...
            s.strats["MOMENTUM"] = s.strats.get("MOMENTUM", "monitoring")

        # ── S6: OPENDRIVE (early market momentum, bonus strategy) ──
        # v11.1 NEW: Backtest proven 7t, 57%WR, +$162 (low volume bonus)
        sig = s.s6.check(m, s.feed, s.trend)
        if sig and time.time() - s.cd.get(f"od:{slot_key}", 0) > 30 and not s.sizer.is_paused("OPENDRIVE"):
            p = sig["p"]
            ok, reason, same_count = allowed("OPENDRIVE", sig["dir"], p)
            if not ok:
                s.strats["OPENDRIVE"] = reason
            else:
                sz = s.c.get_base_size("OPENDRIVE", s.risk.show_bal)
                sz = sz * market_penalty * _cl_mult
                sz = min(sz, av, max_market_risk - market_risk, hard_max)
                if sz >= 1.0 and 0.10 <= p <= 0.50:
                    s.strats["OPENDRIVE"] = f"ACTIVE {sig['dir']} drive={sig.get('drive',0)*100:.2f}%"
                    sh = max(sz / p, 5.0)
                    if sh * p > av: sh = av / p
                    s.dash.ev(f"[{slot_label}·ODRIVE] {sig['dir']} ${sz:.2f} @${p:.2f}")
                    oid, actual_shares = s.ex.order(m, sig["yes"], p, sh, mode="maker")
                    if oid:
                        t = Trd(datetime.now(timezone.utc), "OPENDRIVE", m.slug, sig["dir"], p, sz, oid=oid)
                        s.risk.open(t, market_end=m.end, actual_shares=actual_shares, entry_regime=s.trend.regime if s.trend else "UNKNOWN"); s.cd[f"od:{slot_key}"] = time.time()
                        s._recent_entries.append((time.time(), t.side, slot_key))
                        if m.cid: s._traded_cids.add(m.cid); s._save_traded_cids()
                        s._beep()
                else: s.strats["OPENDRIVE"] = f"signal! {sig['dir']} p=${p:.2f}"
        else:
            s.strats["OPENDRIVE"] = s.strats.get("OPENDRIVE", "monitoring")
