...
class S_Arb:
    ...
    def check(s, m, trend):
        ...
        pair = yp + np_
        # EXP: More aggressive — sum < $0.98 (2% edge) instead of 4%
        if pair >= 0.98: return None
        buy_yes = yp < np_
        price = yp if buy_yes else np_
        # EXP: cap at $0.45
        if price < 0.05 or price > 0.45: return None
...
class S_Latency:
    """LATENCY ARB — EXP: Buy the WINNING side fast.
    The real edge: BTC moved but Polymarket is slow to reprice.
    Buy the side that SHOULD win, even at $0.50-$0.65.
    v5 bought at $0.37-$0.55 and was profitable because the
    market catches up or it resolves as winner ($1)."""
    ...
    def check(s, m, f, trend):
        ...
        # EXP: 0.07% threshold
        if abs(chg) < 0.0007: return None

        up = chg > 0
        # Buy the WINNING side — BTC up = YES wins, BTC down = NO wins
        target_price = m.yes_p if up else m.no_p

        # EXP: Allow up to $0.65 — v5 made money at these prices
        if target_price > 0.65 or target_price < 0.10: return None

        # Confidence
        confidence = min(0.90, 0.55 + abs(chg) * 80)
...
class S_Momentum:
    """VELOCITY — EXP: Detect fast BTC spikes, buy winning side.
    Same fix as Latency: buy the side that should WIN, not the cheap side."""
    ...
    def check(s, m, f, trend):
        ...
        # EXP: lower thresholds
        thresholds = {6: 0.0010, 10: 0.0015, 20: 0.0020}
        if abs(best_chg) < thresholds.get(best_window, 0.0015): return None

        up = best_chg > 0
        # Buy the WINNING side
        target_price = m.yes_p if up else m.no_p

        # EXP: Allow up to $0.65
        if target_price > 0.65 or target_price < 0.10: return None

        velocity = abs(best_chg) / (best_window / 2)
        conf = min(0.90, 0.60 + velocity * 400)
...
class S_Flash:
    """ACCUMULATOR — EXP: Buy any cheap side, less restrictive.
    v5 bought at $0.23-$0.37 and won. Don't require reversal signs,
    just buy cheap sides. The R:R does the work."""
    def __init__(s, c): s.c = c
    def check(s, m, f, trend):
        ...
        # Buy whichever side is cheap — $0.05-$0.40
        yes_cheap = 0.05 <= m.yes_p <= 0.40
        no_cheap = 0.05 <= m.no_p <= 0.40

        if not yes_cheap and not no_cheap: return None

        # Pick the cheaper side
        if yes_cheap and no_cheap:
            # Both cheap (rare) — pick the one more likely to win
            if f.chg(60) > 0:
        ...
        if yes_cheap:
            # YES is cheap — buy if ANY positive sign
            if f.chg(30) > -0.001 or trend.trend_dir >= 0 or m.yes_p < 0.25:
                return {"s": "FLASH", "dir": "YES", "yes": True, "price": m.yes_p, "sz": 0}

        if no_cheap:
            # NO is cheap — buy if ANY positive sign
            if f.chg(30) < 0.001 or trend.trend_dir <= 0 or m.no_p < 0.25:
                return {"s": "FLASH", "dir": "NO", "yes": False, "price": m.no_p, "sz": 0}
...
class S_Squeeze:
    ...
    def check(s, m, f, trend):
        ...
        # EXP: Wider window — last 7 minutes, up to $0.30
        if tl > 420 or tl < 60: return None

        yes_cheap = 0.05 <= m.yes_p <= 0.30
        no_cheap = 0.05 <= m.no_p <= 0.30
        if not yes_cheap and not no_cheap: return None

        # 45-second cooldown
        if time.time() - s._last_signal_time < 45: return None

        # YES very cheap → buy if BTC turning up
        if yes_cheap:
        ...
        # NO very cheap → buy if BTC turning down
        if no_cheap:
...
class Dash:
    ...
    def render(s, c, conn, f, risk, mkt, strats, scores, orders, poly_pos,
        ...
        # ╔══ HEADER ══╗
        print(f"\n  {H1}╔{'═'*60}╗{R}")
        print(f"  {H1}║  POLYMARKET BTC BOT v6.2-EXP       {DIM}{now}  ⏱ {rt}{R}  {H1}║{R}")
        print(f"  {H1}╚{'═'*60}╝{R}")
...
# ─── MAIN BOT v6 ───
class Bot:
    HISTORY_FILE = "trade_history_exp.txt"
    ...
    def _load_traded_cids(s):
        ...
        try:
            if os.path.exists("traded_cids_exp.json"):
                with open("traded_cids_exp.json", "r") as f:
                    data = json.load(f)
    ...
    def _save_traded_cids(s):
        ...
        try:
            with open("traded_cids_exp.json", "w") as f:
    ...
    def _auto_redeem(s):
        ...
        try:
            # Only try 5 CIDs at a time to avoid rate limits
            batch = list(s._traded_cids)[:5]
    ...
    def _trade(s, m):
        ...
        if open_count >= 5: return  # max 5 positions per market
        ...
        if sig and time.time() - s.cd.get("flash", 0) > 45 and not s.sizer.is_paused("FLASH") and not bad_hour:
            p = sig["price"]
